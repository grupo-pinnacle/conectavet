  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isForgotPassword) {
      ...
      try {
        ...
        const msg = (
          err as { response?: { data?: { message?: string } } } | null
        )?.response?.data?.message;
        setError(msg || "Error al solicitar la recuperación.");
      } finally {
        setLoading(false);
      }
      return;
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } } | null)
        ?.response?.data?.message;
      ...
      <div className="mx-auto grid min-h-screen max-w-7xl grid-cols-1 items-center gap-12 px-6 py-12 lg:grid-cols-2 lg:px-12">
        ...
        <div className="mx-auto w-full max-w-md">
          <div className="rounded-xl border border-border bg-white p-8 shadow-overlay md:p-12">
            ...
            {successMsg && (
              <div className="mb-5 rounded-lg bg-green-50 p-4 text-sm font-semibold text-teal-700 border border-green-200">
                {successMsg}
              </div>
            )}
