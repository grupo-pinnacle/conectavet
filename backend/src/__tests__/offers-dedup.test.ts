...
jest.mock("../modules/consultations/chat.gateway.js", () => ({
  getIO: jest.fn(() => ({
    to: (room: string) => mockTo(room),
  })),
}));
...
describe("BUG-002 Backend — Deduplicación de tokens Expo Push", () => {
  ...
  it("notifyVetsOnline deduplica tokens entre múltiples veterinarios antes de invocar Expo Push", async () => {
    ...
    await notifyVetsOnline("consultation_new", "Nueva consulta", "En cola", {
      consultationId: "c-2",
    });
  ...
  it("notifyVetsOnline emite la notificación por socket a la sala vets-online", async () => {
    (prisma.user.findMany as jest.Mock).mockResolvedValue([
      { id: "vet-1" },
      { id: "vet-2" },
    ]);
