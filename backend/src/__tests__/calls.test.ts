...
beforeAll(async () => {
  ...
  const stranger = await prisma.user.create({
    data: {
      email: `${prefix}-stranger@test.com`,
      password: hashed,
      role: "CLIENT",
    },
  });
  ...
  strangerToken = jwt.sign(
    { userId: stranger.id, email: stranger.email, role: "CLIENT" as Role },
    process.env.JWT_SECRET as string,
    { expiresIn: "7d" },
  );
...
afterAll(async () => {
  await prisma.message.deleteMany({
    where: { consultation: { client: { email: { startsWith: prefix } } } },
  });
  await prisma.consultation.deleteMany({
    where: { client: { email: { startsWith: prefix } } },
  });
