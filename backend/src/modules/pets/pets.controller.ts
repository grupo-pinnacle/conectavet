...
/**
 * Tiempo real: notifica a los sockets del dueño que una mascota cambió,
 * así las pantallas refrescan sin polling ni recargar.
 */
function emitPetUpdate(ownerId: string, action: 'create' | 'update' | 'delete' | 'restore', pet: unknown) {
  try {
    getIO()?.to(`user:${ownerId}`).emit('pet:updated', { action, pet });
  } catch {
    // Socket no inicializado (ej. tests): ignorar
  }
}
...
const createPetSchema = z.object({
  ...
  age: z.coerce.number().int().nonnegative('La edad debe ser un número positivo').optional(),
...
export async function createPetController(req: RequestWithUser, res: Response) {
  try {
    ...
    emitPetUpdate(req.user.userId, 'create', pet);
...
export async function updatePetController(req: RequestWithUser, res: Response) {
  try {
    ...
    emitPetUpdate(req.user.userId, 'update', updated);
...
export async function deletePetController(req: RequestWithUser, res: Response) {
  try {
    ...
    emitPetUpdate(req.user.userId, 'delete', { id: req.params.id });
...
export async function restorePetController(req: RequestWithUser, res: Response) {
  try {
    ...
    emitPetUpdate(req.user.userId, 'restore', pet);
    return res.status(200).json({ success: true, data: pet });
  } catch (error) {
    return handleError(error, res);
  }
