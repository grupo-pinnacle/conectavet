...
describe('WebSocket: entrega en tiempo real (T-01)', () => {
  ...
  test('dedup por clientMsgId: reenvío devuelve skipped:true', async () => {
    ...
    await delay(150);
    const first: any = await emitMessage(sock, { consultationId: consult.id, content: 'dup', clientMsgId: 'ws-dup-1' });
    const second: any = await emitMessage(sock, { consultationId: consult.id, content: 'dup', clientMsgId: 'ws-dup-1' });
