...
const statusColors: Record<string, string> = {
  ...
  Confirmada: "bg-blue-50 text-[#2563EB]",
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[#475569]">Gestiona tus consultas y horarios</p>
        </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-[#CBD5E1] bg-white shadow-sm">
        <div className="flex min-w-[490px] border-b border-[#CBD5E1]">
          {weekDays.map((day) => (
            <button
              key={day}
        ...
        <div className="p-4 md:p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button className="rounded-lg bg-[#2563EB] px-3 py-2 text-xs font-bold text-white transition-opacity hover:opacity-90 md:px-4">
              + Nueva
            </button>
          </div>
          ...
          <div className="space-y-3">
            {dayAppointments.map((apt) => (
              <div
                ...
                className="flex items-center gap-3 rounded-lg border border-[#CBD5E1] p-3 transition-colors hover:bg-gray-50 md:gap-4 md:p-4"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-xs font-bold text-[#2563EB] md:h-12 md:w-12 md:text-sm">
                  {apt.time}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-[#94A3B8]">{apt.owner}</p>
                </div>
                <div className="hidden gap-2 md:flex">
                  <button className="rounded-lg border border-[#CBD5E1] px-3 py-1.5 text-xs font-semibold text-[#475569] hover:bg-gray-50">
                    Detalle
                  </button>
                  <button className="rounded-lg bg-[#2563EB] px-3 py-1.5 text-xs font-bold text-white transition-opacity hover:opacity-90">
                    Atender
                  </button>
                </div>
              </div>
